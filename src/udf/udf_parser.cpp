...
UDFParser::UDFParser(common::ManagedPointer<transaction::TransactionContext> txn) {
...
std::unique_ptr<FunctionAST> UDFParser::ParseUDF(std::string func_body, std::string func_name,
...
std::unique_ptr<ExprAST> UDFParser::ParseIdentifierExpr() {
  ...
  return std::make_unique<CallExprAST>(id_name, std::move(args), func_name_,
                                        args_type_);
}
...
std::unique_ptr<ExprAST> UDFParser::ParseIfExpr() {
  ...
  return std::make_unique<IfExprAST>(std::move(if_cond), std::move(then_stmt),
                                      std::move(else_stmt));
}
...
std::unique_ptr<FunctionAST> UDFParser::ParseDefinition() {
  ...
  if (auto expr = ParseExpression()) {
    return std::make_unique<FunctionAST>(std::move(expr));
  }
  return nullptr;
}
