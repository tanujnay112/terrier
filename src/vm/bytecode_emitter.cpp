...
void BytecodeEmitter::EmitDeref(Bytecode bytecode, LocalVar dest,
                                LocalVar src) {
  TERRIER_ASSERT(bytecode == Bytecode::Deref1 || bytecode == Bytecode::Deref2 ||
...
void BytecodeEmitter::EmitAssign(Bytecode bytecode, LocalVar dest,
                                 LocalVar src) {
  TERRIER_ASSERT(bytecode == Bytecode::Assign1 || bytecode == Bytecode::Assign2 ||
...
void BytecodeEmitter::EmitCall(FunctionId func_id,
  ...
  TERRIER_ASSERT(params.size() < std::numeric_limits<u16>::max(),
             "Too many parameters!");
...
void BytecodeEmitter::EmitJump(BytecodeLabel *label) {
  ...
  if (label->is_bound()) {
    ...
    TERRIER_ASSERT(delta < std::numeric_limits<i32>::max(),
               "Jump delta exceeds 32-bit value for jump offsets!");
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1) {
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 1,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
                           LocalVar operand_2) {
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 2,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
  ...
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 3,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
                           LocalVar operand_2, LocalVar operand_3,
                           LocalVar operand_4) {
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 4,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
  ...
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 5,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
                           ...
                           LocalVar operand_4, LocalVar operand_5,
                           LocalVar operand_6) {
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 6,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
  ...
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 7,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
...
void BytecodeEmitter::Emit(Bytecode bytecode, LocalVar operand_1,
                           ...
                           LocalVar operand_6, LocalVar operand_7,
                           LocalVar operand_8) {
  TERRIER_ASSERT(Bytecodes::NumOperands(bytecode) == 8,
             "Incorrect operand count for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 0) == OperandType::Local,
             "Incorrect operand type at index 0 for bytecode");
  TERRIER_ASSERT(Bytecodes::GetNthOperandType(bytecode, 1) == OperandType::Local,
