...
class CodeBlock {
 public:
  ...
  void Append(ast::Stmt *stmt) {
    code_.emplace_back(stmt);
  }

  void Append(CodeBlock *stmt) {
    code_.emplace_back(stmt);
  }
  ...
  class CodeWrapper {
   public:
    ...
    ast::Stmt *Compile(ast::AstNodeFactory *factory, util::Region *region) const {
      switch (type_) {
        case SourceType::STMT: return reinterpret_cast<ast::Stmt *>(source_);
        case SourceType::BLOCK: return reinterpret_cast<CodeBlock *>(source_)->true_start_->Compile(factory, region);
      }
    }
