...
std::unique_ptr<StmtAST> PLpgSQLParser::ParseSQL(const nlohmann::json &sql_stmt) {
  ...
  std::unordered_map<std::string, std::pair<std::string, size_t>> query_params;
    ...
    try {
      // TODO(Matt): I don't think the binder should need the database name. It's already bound in the ConnectionContext binder::BindNodeVisitor visitor(accessor_, db_oid_);
  ...
  return std::unique_ptr<SQLStmtAST>(new SQLStmtAST(std::move(parse_result), std::move(var_name), std::move(query_params)));
  //  return nullptr;
}
...
std::unique_ptr<ExprAST> PLpgSQLParser::ParseExpr(common::ManagedPointer<parser::AbstractExpression> expr) {
  if (expr->GetExpressionType() == parser::ExpressionType::COLUMN_VALUE) {
    ...
    if(cve->GetTableName().empty()) {
      return std::unique_ptr<VariableExprAST>(
          new VariableExprAST(cve->GetColumnName()));
    }else{
      auto vexpr = std::unique_ptr<VariableExprAST>(
          new VariableExprAST(cve->GetTableName()));
      return std::unique_ptr<MemberExprAST>(new MemberExprAST(
            std::move(vexpr), cve->GetColumnName()));
    }
